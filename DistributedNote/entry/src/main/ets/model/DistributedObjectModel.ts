...
export default class DistributedObjectModel {
    ...
    update(index: number, title: string, content: string, mark: number, intro: string, prog: number, fontsize: number) {
        Logger.info(TAG, `doUpdate,${title},${index}`)
        let documents = this.distributedObject.documents
        documents[index] = {
            title: title, content: content, mark: mark, intro: intro, prog: prog, fontsize: fontsize
        }
    ...
    add(title: string, content: string, mark: number, intro: string, prog: number, fontsize: number) {
